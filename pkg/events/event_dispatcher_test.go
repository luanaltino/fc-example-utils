...
func (m *MockHandler) Handle(wg *sync.WaitGroup, event EventInterface) {
	m.Called(event)
	wg.Done()
}
...
func (h *TestEventHandler) Handle(wg *sync.WaitGroup, event EventInterface) {
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_ClearByEvent() {
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	err = suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler2)
	suite.Nil(err)
	...
	suite.eventDispatcher.ClearByEvent(suite.event.GetName())
	suite.Nil(suite.eventDispatcher.handlers[suite.event.GetName()])
}
